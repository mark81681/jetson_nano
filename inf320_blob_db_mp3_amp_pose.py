...
net = jetson_inference.detectNet(argv=["--model=/home/jetson/vigil/mb2-ssd-lite_320.onnx", f"--labels={LABELS_PATH}", "--input-blob=input_0", "--output-cvg=scores", "--output-bbox=boxes", "--threshold=0.25", "--verbose"])
...
def crop_save_and_upload(img, dt_str, bbox_xyxy, prefix="intrusion", persist_dir=CAPTURE_DIR):
    ...
    filename = f"{prefix}_{dt_str}.jpg"; local_path = os.path.join(persist_dir, filename)
...
try:
    while True:
        ...
        for t in tracks:
            ...
            # === PoseNet in human bbox (overlay only) ===
            if ENABLE_POSENET and pose_net is not None and HUMAN_ID is not None:
                if t.class_id == HUMAN_ID and (frame_idx % POSENET_PER_HUMAN_EVERY_N_FRAMES == 0):
                    try:
                        ...
                        # crop에만 PoseNet 그리기
                        poses = pose_net.Process(crop_gpu, overlay=POSE_OVERLAY_IN_BOX)

                        # 그려진 crop을 원본에 합성
                        try:
                            jetson_utils.cudaOverlay(crop_gpu, img, bx1, by1)
                        except Exception as e_overlay:
                            # 폴백: overlay 없이 포즈 받고, 키포인트만 직접 찍기
                            try:
                                poses = pose_net.Process(crop_gpu, overlay="none")
                                for p in poses:
                                    for k in p.Keypoints:
                                        kx = int(bx1 + k.x); ky = int(by1 + k.y)
                                        jetson_utils.cudaDrawCircle(img, (kx, ky), 2, (0, 255, 255, 255))
                            except Exception as e_fb:
                                print("[POSE][fallback] draw failed:", e_fb)
                    except Exception as e:
