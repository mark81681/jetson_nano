...
display_rtmp  = jetson_utils.videoOutput("rtmp://agrilook-be-stream.koreacentral.cloudapp.azure.com:1935/live/jetson2")
...
COORDS_PATH = os.path.join(BASE_DIR, "coords.json")

# 좌표 원본/스케일/리로드 상태
coords_raw = None       # {"ROI_POLY":[[x,y],...], "ROI_CLOSED":bool, "GEOFENCE_LINE":[[x,y],[x,y]], "REF_W":int, "REF_H":int}
coords_scaled = None    # {"roi":[(x,y),...], "closed":bool, "line":((x,y),(x,y))}
coords_mtime = 0.0
global REF_W, REF_H
REF_W, REF_H = None, None  # json에 기준 해상도가 있으면 사용
...
def _load_coords_json(path: str):
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    return data

def _infer_ref_size(data):
    # json에 REF_W/REF_H가 있으면 사용, 없으면 좌표의 최대값으로 추정
    rw = data.get("REF_W"); rh = data.get("REF_H")
    if isinstance(rw, int) and isinstance(rh, int) and rw > 0 and rh > 0:
        return rw, rh
    pts = []
    pts += data.get("ROI_POLY", [])
    line = data.get("GEOFENCE_LINE", [])
    pts += line
    maxx = max((p[0] for p in pts), default=0)
    maxy = max((p[1] for p in pts), default=0)
    # 좌표가 0~max 이면 실제 폭/높이는 max+1 로 보는 게 자연스럽다.
    return max(1, maxx + 1), max(1, maxy + 1)

def _scale_points(pts, src_w, src_h, dst_w, dst_h):
    sx = float(dst_w) / float(max(1, src_w))
    sy = float(dst_h) / float(max(1, src_h))
    return [(int(round(x*sx)), int(round(y*sy))) for (x,y) in pts]

def _build_scaled_coords(raw, dst_w, dst_h):
    rw, rh = _infer_ref_size(raw)
    roi_poly = raw.get("ROI_POLY", [])
    roi_closed = bool(raw.get("ROI_CLOSED", False))
    gf_line = raw.get("GEOFENCE_LINE", [])

    roi_scaled = _scale_points(roi_poly, rw, rh, dst_w, dst_h)
    line_scaled = _scale_points(gf_line, rw, rh, dst_w, dst_h)
    return {
        "roi": roi_scaled,
        "closed": roi_closed,
        "line": tuple(line_scaled) if len(line_scaled) >= 2 else None
    }

def draw_poly(img, pts, closed=False, color=(0, 200, 255, 255), thick=2):
    if not pts or len(pts) < 2:
        return
    # 선 두께를 흉내내기 위해 평행선 조금 겹치기 (cudaDrawLine은 두께 옵션 없음)
    for i in range(len(pts) - 1):
        x1, y1 = pts[i]; x2, y2 = pts[i+1]
        for t in range(thick):
            jetson_utils.cudaDrawLine(img, (x1, y1+t), (x2, y2+t), color)
    if closed:
        x1, y1 = pts[-1]; x2, y2 = pts[0]
        for t in range(thick):
            jetson_utils.cudaDrawLine(img, (x1, y1+t), (x2, y2+t), color)

def draw_line(img, p1, p2, color=(255, 80, 80, 255), thick=2):
    x1, y1 = p1; x2, y2 = p2
    for t in range(thick):
        jetson_utils.cudaDrawLine(img, (x1, y1+t), (x2, y2+t), color)
...
try:
    while True:
        ...
        # ---- coords.json 핫 리로드 (파일이 있으면 오버레이 준비) ----
        try:
            if os.path.exists(COORDS_PATH):
                mtime = os.path.getmtime(COORDS_PATH)
                if (coords_raw is None) or (mtime != coords_mtime):
                    new_raw = _load_coords_json(COORDS_PATH)
                    # 기준 해상도(optional) 저장
                    REF_W, REF_H = _infer_ref_size(new_raw)
                    # 현재 프레임 크기에 맞춰 스케일
                    new_scaled = _build_scaled_coords(new_raw, w, h)

                    coords_raw = new_raw
                    coords_scaled = new_scaled
                    coords_mtime = mtime
                    print(f"[COORDS] loaded & scaled (ref={REF_W}x{REF_H} -> frame={w}x{h})")
        except Exception as e:
            # 좌표 파일 문제는 치명적이지 않으니 경고만
            print(f"[COORDS] load/scale error: {e}")
        ...
        for t in tracks:
            ...
            if elapsed >= need_sec and tid not in captured_ids:
                ...
                # "YYYYMMDD-HHMMSS" KST(UTC+9)로 저장
                dt_str = datetime.now(KST).strftime("%Y%m%d-%H%M%S")
                crop_local, crop_url = crop_save_and_upload(img, dt_str, (x1, y1, x2, y2), prefix=label_prefix(label, crop=True))
                full_local, full_url = full_save_and_upload(img, dt_str, prefix=label_prefix(label, crop=False))

                # === DB로 class + datetime(문자열) 저장, _id는 시퀀스(0001...) ===
                try:
                    # 1) 새 번호 뽑기 (원자적 증가)
                    seq_str = next_id_in_same_collection(width=4)  # 0001, 0002, ...

                    conf_pct = int(round(t.score * 100))
                    conf_str = "{}%".format(conf_pct)

                    # 3) DB 저장 (요청대로 class와 datetime만)
                    col.insert_one({
                        "_id": seq_str,
                        "class": label,
                        "confidence": conf_str,
                    ...
                    # 동시 삽입으로 같은 _id가 생기는 아주 드문 경우: 다시 한 번만 재시도
                    try:
                        seq_str = next_id_in_same_collection(width=4)
                        dt_str = datetime.now(KST).strftime("%Y%m%d-%H%M%S")  # ← KST로 통일
                        conf_pct = int(round(t.score * 100))
                        conf_str = f"{conf_pct}%"
                        ...
                        col.insert_one({
                            "_id": seq_str,
                            "class": label,
                            "confidence": conf_str,   # ← 누락 복구
                            "datetime": dt_str,
                            "farm_id": FARM_ID,
                        })
                        print(f"[DB] inserted (retry) _id={seq_str} class={label} conf={conf_str} datetime={dt_str}")
        ...
        if coords_scaled is not None:
            roi_pts = coords_scaled.get("roi") or []
            gf = coords_scaled.get("line")
            # ROI: 시안색 / 지오펜스: 빨강
            draw_poly(img, roi_pts, closed=coords_scaled.get("closed", False), color=(0,200,255,255), thick=2)
            if gf and len(gf) == 2:
                draw_line(img, gf[0], gf[1], color=(255,80,80,255), thick=2)
            # (선택) 꼭짓점 점 표시
            for (px,py) in roi_pts:
                jetson_utils.cudaDrawCircle(img, (int(px), int(py)), 2, (0, 255, 180, 255))
            if gf and len(gf) == 2:
                jetson_utils.cudaDrawCircle(img, (int(gf[0][0]), int(gf[0][1])), 3, (255, 120, 120, 255))
                jetson_utils.cudaDrawCircle(img, (int(gf[1][0]), int(gf[1][1])), 3, (255, 120, 120, 255))
        ...
        if time.time() - start > 120: # Timeout 설정 (필요시 조정)
            print("[TIMEOUT] Timeout exit ==========================="); break
