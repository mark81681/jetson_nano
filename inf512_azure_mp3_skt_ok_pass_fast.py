...
# === [변경점] 업로드/DB 백그라운드 워커 ===
upload_db_q = Queue(maxsize=128)
bg_thread = None  # 시작은 모델 로딩 이후에

def upload_db_worker():
    while True:
        job = upload_db_q.get()
        if job is None:
            upload_db_q.task_done()
            break
        try:
            # Blob 업로드
            try:
                if job.get("crop_path"):
                    upload_file_to(job["crop_path"], job["crop_name"], container=CONT_CROP, sas_url=SAS_CROP)
                if job.get("full_path"):
                    upload_file_to(job["full_path"], job["full_name"], container=CONT_FULL, sas_url=SAS_FULL)
            except Exception as e:
                print(f"[BLOB/bg] upload failed: {e}")
            ...
            try:
                if db_ok and col is not None:
                    ...
                    col.insert_one({
                        "_id": seq_str,
                        "class": job["label"],
                        "confidence": job["conf_str"],
                        "datetime": job["dt_str"],
                        "farm_id": job["farm_id"],
                    })
                    print(f"[DB/bg] inserted _id={seq_str} class={job['label']} conf={job['conf_str']} datetime={job['dt_str']}")
            except DuplicateKeyError:
                pass
            except Exception as e:
                print(f"[DB/bg] insert failed: {e}")
        finally:
            upload_db_q.task_done()
...
class BYTETrackerKF:
    ...
    def _greedy_match(iou_mat, iou_th):
        ...
        u_b = [j for j in range(iou_mat.shape[1]) if i not in matched_b]
...
# === [변경점] 길이 파싱 제거: 즉시 재생만 수행 (프레임 블로킹 감소)
...
# ====== 클래스별 운용 튜닝 ======
CLASS_CONF_TH = { "Human": 0.35, "Wild Boar": 0.40, "Wild Rabbit": 0.40, "Bird": 0.35, "Siberian Chipmunk": 0.35, "Squirrel": 0.9, "Weasel": 0.45, "Leopard Cat": 0.40, "Racoon": 0.50, "Water Deer": 0.40, "Dog": 0.90, }
...
RISK_POLICY = { "high": {"persist": 1.0}, "medium": {"persist": 2.5}, "low": {"persist": 3.0} }
...
net = jetson_inference.detectNet(argv=["--model=/home/jetson/vigil/mobilenet/mb2-ssd-lite_512_full.onnx", f"--labels={LABELS_PATH}", "--input-blob=input_0", "--output-cvg=scores", "--output-bbox=boxes", "--threshold=0.25", "--verbose"])
...
# === [변경점] 백그라운드 워커 스레드 시작 ===
bg_thread = Thread(target=upload_db_worker, daemon=True)
bg_thread.start()
...
# === [변경점] 저장 전용 헬퍼 추가 (업로드 없음) ===
def crop_save_only(img, dt_str, bbox_xyxy, prefix="intrusion", persist_dir=CAPTURE_DIR):
...
def full_save_only(img, dt_str, prefix="full", persist_dir=CAPTURE_DIR):
    os.makedirs(persist_dir, exist_ok=True)
    try:
        kind, det_class = prefix.split("_", 1)
    except ValueError:
    ...
    while True:
        ...
        for t in tracks:
            ...
            if (not ok_silence) and (elapsed >= need_sec) and (tid not in captured_ids):
                try:
                    ...
                    # === [변경점] 메인 쓰레드에서는 "저장만" 수행 (업로드/DB는 워커로)
                    crop_local = crop_save_only(
                        img, dt_str, (x1, y1, x2, y2),
                        prefix=label_prefix(label, crop=True)
                    )
                    full_local = full_save_only(
                        img, dt_str,
                        prefix=label_prefix(label, crop=False)
                    )

                    # === [변경점] 업로드/DB 작업을 큐에 위임
                    try:
                        upload_db_q.put_nowait({
                            "crop_path": crop_local,
                            "crop_name": os.path.basename(crop_local),
                            "full_path": full_local,
                            "full_name": os.path.basename(full_local),
                            "label": label,
                            "conf_str": conf_str,
                            "dt_str": dt_str,
                            "farm_id": FARM_ID,
                        ...
                        print(f"[BG-QUEUE] push failed: {e}")
        ...
        if chosen_sound:
            ...
            if (not audio_playing) and (now - last_ts) >= AUDIO_COOLDOWN_SEC:
                ...
                play_mp3_once(mp3_file, volume=100)
                print(f"[AUDIO] using file: {mp3_file}")
                print(f"[AUDIO] {classes_str} detected -> {os.path.basename(mp3_file)} playing....")
